...
実行時にアプリケーションへコンポーネントを追加するには、`createComponent` APIを使用して[プログラムでレンダリング](guide/components/programmatic-rendering)できます。
この方法では、変更検知が動作するようコンポーネントのホストビューを`ApplicationRef`にアタッチする、入力を設定する、出力を購読する、コンポーネントが削除されるときにビューをデタッチしてクリーンアップするといった、周辺のインフラストラクチャの管理をすべて自分で担う必要があります。

Angularカスタム要素を使用すると、このインフラストラクチャをすべて自動的に提供することで、プロセスがよりシンプルかつ透過的になります。必要なのは、希望するイベント処理の種類を定義することだけです。
