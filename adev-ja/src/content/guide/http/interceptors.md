...
## インターセプターの定義 {#defining-an-interceptor}
...
## インターセプターの構成 {#configuring-interceptors}
...
### レスポンスイベントのインターセプト {#intercepting-response-events}
...
## インターセプターでの依存性の注入 {#dependency-injection-in-interceptors}
...
## リクエストとレスポンスのメタデータ {#request-and-response-metadata}
...
### コンテキストトークンの定義 {#defining-context-tokens}
...
### インターセプターでのトークンの読み込み {#reading-the-token-in-an-interceptor}
...
### リクエストの作成時のコンテキストトークンの設定 {#setting-context-tokens-when-making-a-request}
...
### リクエストコンテキストは変更可能 {#the-request-context-is-mutable}
...
## リダイレクト情報の操作 {#working-with-redirect-information}
